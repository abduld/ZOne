...
__global__ void gpuEuropeanMonteCarlo(float * call, 
...
void EuropeanMonteCarlo(zMemoryGroup_t out, zMemoryGroup_t S, zMemoryGroup_t X,
	zMemoryGroup_t T, zMemoryGroup_t R, zMemoryGroup_t V, zMemoryGroup_t Q) {
  ...
  gpuEuropeanMonteCarlo<<<gridDim,blockDim, 0, strm>>>(
    ...
    (float*)zMemoryGroup_getDeviceMemory(Q),
    1,
    2,
    len
  );
...
int main(int argc, char *argv[]) {
  ...
  zMapGroupFunction_t mapFun = zMapGroupFunction_new(st, "european", EuropeanMonteCarlo);
