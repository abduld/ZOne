...
__global__ void gpuBlackScholes(float * call, float * S, float * X,
	float * T, float * r, float * sigma, int len)
{
	...
	float d1=(log(S[ii]/X[ii])+(r[ii]+(sigma[ii]*sigma[ii])/2)*T[ii])/(sigma[ii]*sqrt(T[ii]));
...
void BlackSholes(zMemory_t out, zMemory_t S, zMemory_t X,
	zMemory_t T, zMemory_t r, zMemory_t sigma) {
...
size_t dim = 1 << atoi(argv[1]);
  ...
  cudaMemcpy(dSMem, hSMem, sizeof(float)*inputLength, cudaMemcpyHostToDevice);
  cudaMemcpy(dXMem, hXMem, sizeof(float)*inputLength, cudaMemcpyHostToDevice);
  cudaMemcpy(dTMem, hTMem, sizeof(float)*inputLength, cudaMemcpyHostToDevice);
  cudaMemcpy(dRMem, hRMem, sizeof(float)*inputLength, cudaMemcpyHostToDevice);
  cudaMemcpy(dQMem, hQMem, sizeof(float)*inputLength, cudaMemcpyHostToDevice);

  dim3 blockDim(BLOCK_DIM_X);
  dim3 gridDim(zCeil(inputLength, blockDim.x));
  gpuBlackScholes<<<gridDim,blockDim>>>(dOutputMem, dSMem, dXMem, 
  	dTMem, dRMem, dQMem, inputLength);

  cudaMemcpy(hOutputMem, dOutputMem, sizeof(float)*inputLength, cudaMemcpyDeviceToHost);
