...
#define zStringBuffer_setCurrentLength(sb, val)                                \
  (zStringBuffer_getCurrentLength(sb) = val)
#define zStringBuffer_setBuffer(sb, val) (zStringBuffer_getBuffer(sb) = val)
#define zStringBuffer_setBufferChar(sb, ii, val)                               \
  (zStringBuffer_getBufferChar(sb, ii) = val)
#define zStringBuffer_setMaxLength(sb, val)                                    \
  (zStringBuffer_getMaxLength(sb) = val)
...
static zBool_t zStringBuffer_expand(zStringBuffer_t sb, size_t len) {
  if (sb) {
    if (zStringBuffer_getCurrentLength(sb) + len <
        zStringBuffer_getMaxLength(sb) - 1) {
      return zTrue;
    } else {
...
void zStringBuffer_append(zStringBuffer_t sb, const char *msg) {
  if (sb) {
    ...
    if (zStringBuffer_expand(sb, slen)) {
      size_t ii = 0;
      char *buf =
          zStringBuffer_getBuffer(sb) + zStringBuffer_getCurrentLength(sb);
      ...
      *buf = '\0';
      ...
      zStringBuffer_setCurrentLength(sb,
                                     zStringBuffer_getCurrentLength(sb) + slen);

      assert(zStringBuffer_getCurrentLength(sb) <
             zStringBuffer_getMaxLength(sb));
    }
  }
  return;
}
...
zStringBuffer_t zStringBuffer_reverse(zStringBuffer_t from) {
  if (from) {
    zStringBuffer_t to =
        zStringBuffer_initialize(zStringBuffer_getCurrentLength(from) + 1);
    char *toBuf, *fromBuf;
    size_t ii = 0, len;
...
zStringBuffer_t zStringBuffer_take(zStringBuffer_t from, size_t start,
  ...
  if (from == NULL || start >= zStringBuffer_getCurrentLength(from) ||
