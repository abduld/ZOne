...
#define zSuccessQ(err)         (zError_getCode(err) == zSuccess || zError_getCode(err) == zError_uv)
...
#define zError(err, code)        zError_update(err, zError_##code, zFile, zFunction, zLine)
#define zError_uv(err)         zError(err, uv)

extern zError_t zError_new();
extern void zError_delete(zError_t err);

extern void zError_update(zError_t err, zErrorCode_t code, const char * file, const char * fun, int line);
